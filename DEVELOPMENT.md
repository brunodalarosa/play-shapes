...
For Bubbles creature animation review, `godot --path . --rendering-method gl_compatibility --script res://tests/bubbles_creature_motion_visual_check.gd` saves warning-on, two pufferfish jiggle/trail frames, the trail fade, warning-off, and jellyfish breathing captures under ignored `test-results/ps-049/`. Inspect the ordered stills; they show rendered size and states but do not establish motion feel or owner readability/fairness approval.
