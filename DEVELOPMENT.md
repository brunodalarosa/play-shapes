...
For PS-048 animation review, `godot --path . --rendering-method gl_compatibility --script res://tests/bubbles_animation_visual_check.gd` saves small/maximum idle, charge, accepted swipe, active spin, burst, and reformed captures under ignored `test-results/ps-048/`. Inspect the sequence in order; still images cannot establish motion feel.
