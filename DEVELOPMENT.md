...
- `debug/` contains the non-pausing F12 scenario catalog and debug scenes. The launcher remains present in development exports. **Rendered squircle comparison** opens the isolated PS-058 preview: action, front/three-quarter view, ten canonical colors, neutral/blink face, and pause controls. Its 256/128-pixel comparisons pair the PS-057 render with the current 2D squircle. `debug/squircle_preview/manifest.json` maps the copied sheets to the source export's frame counts and ground anchors. This is local art review only; the production renderer and phone client do not use these sheets.
...
The lobby's selection and name previews use the open-hand sprite rotated down beside the character's waist. The Bubbles phone preview keeps its closed-hand sprite.
