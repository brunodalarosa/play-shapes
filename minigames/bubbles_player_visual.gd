...
extends Node2D
## Code-drawn bubble with approved small jellyfish art. Never decides rules.
...
func _draw() -> void:
	...
	for index: int in captured_visual_count:
		...
		draw_texture_rect(SMALL_JELLYFISH, Rect2(center - size * 0.5, size), false, Color(1.0, 1.0, 1.0, 0.82))
	if spinning:
