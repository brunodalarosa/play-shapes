...
function drawCharacter(context, shape, tint, x, y, scale, pose) {
    context.save();
    context.translate(x, y);
...
function renderJoinPreviews(now) {
    ...
    for (const canvas of [selectionPreview, namePreview]) {
        ...
        drawCharacter(context, joinFlow.shape, joinFlow.color, rect.width / 2, rect.height * 0.46 + (dancing ? Math.sin(seconds * 3.4) * 4 : 0), scale, pose);
    }
}
